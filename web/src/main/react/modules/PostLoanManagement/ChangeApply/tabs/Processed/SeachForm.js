...
let SeachForm = React.createClass({
  ...
  componentDidMount() {
    var me = this;
    Utils.ajaxData({
      url: '/modules/common/action/ComboAction/queryDic.htm?typeCode=WORKFLOW_STATE',
      method: 'get',
      type: 'json',
      callback: (result) => {
    ...
    Utils.ajaxData({
     ...
     Utils.ajaxData({
      url: '/modules/common/action/ComboAction/queryDic.htm?typeCode=REPAYMENT_TYPE',
      method: 'get',
      type: 'json',
      callback: (result) => {
        var items = result.data.map((item) => {
  ...
  render() {
    ...
    return (
      <Form inline >
        ...
        <FormItem label="还款方式：">
          <Select style={{ width: "160px" }} {...getFieldProps('repaymentType', { initialValue: '' }) }>
            {this.state.payway}
          </Select>
        </FormItem>
        <FormItem label="流程状态：">
          <Select style={{ width: "160px" }} {...getFieldProps('processStateCode', { initialValue: '' }) }>
