...
var RevieCustumerwWin = React.createClass({
    ...
    handleOk() {//新增催收提交
        ...
        var dunTime;
        if (formeData.dunTime) {
            dunTime = DateFormat.formatTime("yyyy-MM-dd hh:mm:ss", formeData.dunTime)
        }
        var creditConsultFrom={
            repaymentId:props.record.id,
            dunTime:dunTime,
            type:formeData.type,
        ...
        this.props.form.validateFields((errors, values) => {
            if (!!errors) {
                ...
                Utils.ajaxSubmit({
                    me: this,
                    url: '/modules/credit/RepaymentCollectionAction/save.htm',
                    method: 'post',
                    data: {
                        pubDundetail: JSON.stringify(creditConsultFrom),
                    },
                    type: 'json',
                    callback: (result) => {
                        if (result.code == 200) {
            ...
            <Modal title= { props.title } visible= { props.visible } onCancel= { this.handleCancel }   width= "1200"  footer= { modalBtns } >
                <Form horizontal  form={this.props.form} style={{ marginTop: "20px" }}>
                    <div style={{ position: "relative" }}>
                        <Row>
                            <Col span="7">
                                <FormItem  {...formItemLayout} label="催收时间：">
                                    <DatePicker disabled={!props.canEdit} wrapperCol={{ span: "7" }}  {...getFieldProps('dunTime', { rules: [{ required: true, message: '必填', type: "date" }] }) } />
                                </FormItem>
                            </Col>
                            <Col span="7">
                        ...
                        <Row>
                            <Col span="19">
                                <FormItem  {...formItemLayout} label="催收内容："  labelCol={{ span: "3" }}>
