...
export default React.createClass({
  ...
  fetch(params = {}) {
    ...
    var url = '/modules/workflow/ProcessTaskController/queryInstanceXiaHuTasks.htm?isCompleted=false';
    ...
    return (
      <div className="block-panel">
        <div className="actionBtns" style={{ marginBottom: 16 }}>
          <button className="ant-btn" key="1" disabled={!hasSelected} onClick={this.showModal.bind(this, '处理', "", true,true) }>
            处理
          </button>
        </div>
        ...
        <ReviewWin ref="ReviewWin"  visible={state.visible}  title={state.title} hideModal={me.hideModal} record={state.selectedrecord} id={state.id} canEdit={state.canEdit} canUpload={state.canUpload}/>
        <TaskAllocation ref="TaskAllocation"  visible={state.visibleTask} getUserNameList={state.getUserNameList}   title={state.title} hideModal={me.hideModal} record={state.selectedrecord} id={state.id} canEdit={state.canEdit}/>
      </div>
    );
  }
})
