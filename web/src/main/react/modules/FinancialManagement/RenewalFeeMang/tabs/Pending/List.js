...
export default React.createClass({
  ...
  showModal(title, canEdit) {
    ...
    if (title == "展期费收取") {
      Utils.ajaxData({
        url: '/modules/extension/ExtensionAction/getHousExtendedFee.htm',
        data: {
  ...
  fetch(params = {}) {
    ...
    Utils.ajaxData({
      url: '/modules/workflow/ProcessTaskController/queryExtendedfeeTasks.htm?isCompleted=false',
      data: params,
      callback: (result) => {
    ...
    return (
      <div className="block-panel">
        <div className="actionBtns" style={{ marginBottom: 16 }}>
          <button className="ant-btn"  disabled={!hasSelected} onClick={this.showModal.bind(this, '展期费收取', true) }>
            展期费收取
          </button>
        </div>
        ...
        <AddlWin ref="AddlWin"  visible={state.visible} formData={state.formData}   showModal={this.showModal.bind(this, '展期费收取', true) }
