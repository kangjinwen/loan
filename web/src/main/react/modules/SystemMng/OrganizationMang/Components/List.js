...
export default React.createClass({
    ...
    render() {
        ...
        var columns = [{
            ...
            render(text, record) {
                if (record.value!='0') {
                    return <div style={{textAlign: "left"}}><a href="#" onClick={me.delete.bind(me, record)}>删除 </a>
                    </div>
                }
            }
        }];
        return (
            <div className="block-panel">
                <div className="actionBtns" style={{marginBottom: 16}}>
                    ...
                    <button className="ant-btn" disabled={!hasSelected}
                            onClick={this.showModal.bind(this, '增加机构', true)}>
                        增加机构
                    </button>
                </div>
                <Table columns={columns} rowKey={this.rowKey} rowSelection={rowSelection} size="middle" ref="table"
                       pagination={false}
                ...
                <AddInstitution ref="AddInstitution" visible={state.visible} officeData={this.state.data}
                                title={state.title} hideModal={me.hideModal} record={state.record}
