    ...
    render() {
            ...
            current = (
                <Col span="5" >
                    <FormItem  {...formItemLayout} label="逾期金额(元)："  style={{ marginLeft: "-100px", marginTop: "15px" }}>
                        <MicroInput disabled={!props.canEdit} style={{ width: "80%" }} {...getFieldProps('overdueAmounts',
                            {
                                rules: [
            ...
            secured = (
                <Col span="8">
                    <FormItem  {...formItemLayout} label="担保性贷款金额(元)：" style={{ marginLeft: "-20px", marginTop: "15px" }}>
                        <MicroInput disabled={!props.canEdit} style={{ width: "82%" }} {...getFieldProps('securedLoanAmounts',
                            {
                                rules: [
                                    { required: true, message: '必填只能是数字', type: 'float' },
                                    { message: '不能为负数，不能大于10位数，保留两位', pattern: /^\d{1,10}(\.\d{0,2})?$/ },
            ...
            bad = [
                <Col span="7" key='1'>
                    <FormItem  {...formItemLayout} label="呆账笔数：" >
                        <Input disabled={!props.canEdit} style={{ width: "100%" }} {...getFieldProps('badDebtItems',
                            {
                                rules: [
                                    { required: true, message: '必填只能是数字', type: 'float' },
                                    { message: '不能为负数，不能大于10位数，保留两位', pattern: /^\d{1,10}(\.\d{0,2})?$/ },
                ...
                <Col span="7" key='2'>
                    <FormItem  {...formItemLayout} label="呆账金额(元)：">
                        <MicroInput disabled={!props.canEdit} style={{ width: "100%" }} {...getFieldProps('badDebtAmounts',
                            {
                                rules: [
                                    { required: true, message: '必填只能是数字', type: 'float' },
                                    { message: '不能为负数，不能大于10位数，保留两位', pattern: /^\d{1,10}(\.\d{0,2})?$/ },
        ...
        return (
            <Form horizontal  form={this.props.form} style={{ marginTop: "20px" }}>
                ...
                <Row>
                    <Col span="10">
                        <FormItem label={<span className='ant-form-item-required'>证件照片相符：</span>} {...formItemLayout} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} >
                ...
                <Row>
                    <Col span="10">
                        <FormItem label={<span className='ant-form-item-required'>证件信息一致：</span>} {...formItemLayout} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}  >
