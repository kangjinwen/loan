...
export default React.createClass({
  ...
  handleChange(info) {
    console.log(info)
      if(info.file.status  === 'done'){console.log("handleChange",info);
  ...
  render() {
    ...
    if(props.otherType&&btype!='HOUSE'){
     ...
     images = this.state.fileList.map((file,index )=>{
      ...
      return <div className="img" key ={index} >
          <a href={url} target="view_window" >{fileExtension=='jpg'||fileExtension=='JPG'||fileExtension=='JPEG'||fileExtension=='jpeg'?<img src={url} />:<iframe width="100%" height="230px" src={url}></iframe>}</a> 
          {fileExtension=='pdf'?<button key="lookPdf" type="button" className="ant-btn lookPdf" onClick={this.lookPdf.bind(this,url)}>预览</button>:null}
          {canEdit?<button key="delete" type="button" className="ant-btn" onClick={this.deletePic.bind(this,file.id)}><i className="anticon anticon-delete"></i>删除</button>:null} 
      </div>
     }); 
    /*var postDate={
                    rd_operatorid :1,
    ...
    const uploadProps = {
        ...
        beforeUpload(file) {
            ...
            if (!isJPG) {
                if (btype == "Contract_Pic") {
                    message.error('只能上传 JPG 文件或者pdf文件哦！');
                } else {
    ...
    return( <div className="uploadFile">
                ...
                <div className="img">
                  <div className="uploadBg uploadPlus" >
                    {canEdit ? <Upload {...uploadProps}> <a style={{ marginRight: 25, }}></a> </Upload> : null}
                  </div> 
